  ...
  <div class="relative" :class="[isAccordion ? 'bg-night-shade pb-1' : 'bg-night-shade rounded-xl mb-4']">
    <button type="button" class="w-full p-4 text-left" :class="{ 'px-0': isAccordion }" @click="onActive">
      <div>
        <div className="flex items-center text-base font-medium justify-between">
            ...
            <span id="icon-1" class="text-gray-200 transition-transform duration-300">
                <!-- minus sign -->
    ...
    <div
      ...
      :class="[
        ...
        {
          'border-b border-gray-200': isAccordion,
        },
      ]">
      <div class="px-4 pb-4 text-gray-300" :class="{ 'px-0': isAccordion }">
        <slot name="content" />
      </div>
    </div>
  </div>
