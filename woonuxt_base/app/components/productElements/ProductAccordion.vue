<template>
  <div class="relative" :class="[isAccordion ? 'bg-gray-800 dark:bg-night-shade pb-1' : 'bg-gray-800 dark:bg-night-shade rounded-xl mb-4']">
    <button type="button" class="w-full p-4 text-left" :class="{ 'px-0': isAccordion }" @click="onActive">
      <div>
        <div className="flex items-center text-base font-medium justify-between">
            <span class="text-gray-500 dark:text-gray-300">
                <slot name="title" />
            </span>

            <span id="icon-1" class="dark:text-gray-200 transition-transform duration-300">
                <!-- minus sign -->
    ...
    <div
      ...
      :class="[
        ...
        {
          'border-b border-gray-500 dark:border-gray-200': isAccordion,
        },
      ]">
      <div class="px-4 pb-4 text-gray-500 dark:text-gray-300" :class="{ 'px-0': isAccordion }">
        <slot name="content" />
      </div>
    </div>
  </div>
