...
<template>
  <aside v-if="cart" class="bg-white rounded-lg shadow-lg mb-8 w-full min-h-[280px] p-4 sm:p-8 relative md:max-w-md md:top-32 md:sticky">
    ...
    <div class="grid gap-1 text-sm font-semibold text-gray-300">
      <div class="flex justify-between">
        <span>{{ $t('messages.shop.subtotal') }}</span>
        <span class="text-gray-300 tabular-nums" v-html="cart.subtotal" />
      </div>
      <div class="flex justify-between">
      ...
      <div class="flex justify-between mt-4">
        ...
        <span class="text-lg font-bold text-gray-300 tabular-nums" v-html="cart.total" />
      </div>
    </div>
