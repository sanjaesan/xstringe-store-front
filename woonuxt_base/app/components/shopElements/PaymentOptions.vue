...
<template>
  <div class="flex gap-4 leading-tight flex-wrap">
    ...
    <div v-if="activePaymentMethod.description" class="prose block w-full">
      <p class="text-sm text-gray-500 dark:text-gray-300" v-html="activePaymentMethod.description" />
    </div>
  </div>
</template>
...
.option {
  @apply bg-white border rounded-lg text-gray-600 dark:text-gray-400 cursor-pointer flex flex-1 text-sm py-3 px-4 gap-2 items-center hover:border-purple-300;
